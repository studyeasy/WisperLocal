...
5. **Enhance (optional)** — If you turn it on, a small **local** language model (Google's Gemma, via Ollama) rewrites the text into clean, well-structured prose.
...
WisperLocal uses local Whisper models that download on demand (with a progress bar) — from a tiny ~75 MB model for quick notes up to large models for maximum accuracy. The optional writing enhancement runs a small local LLM through [Ollama](https://ollama.com), which the installer can set up for you.
